...
class MyLogFormatter(colorlog.ColoredFormatter):
    ...
    def format(self, record):
        ...
        record_message = '%02d:%02d:%02d.%04d %s %s:%d] %s' % (
            ...
            MyLogFormatter.RANK if MyLogFormatter.RANK is not None else '?',
            record.filename,
            record.lineno,
...
def set_rank(new_rank):
    MyLogFormatter.RANK = new_rank
    logger.info('Logger rank set to %d', new_rank)


def set_allowed_ranks(new_ranks):
    MyLogFormatter.ALLOWED_RANKS = new_ranks
    logger.info('Logger allowed ranks set to %s', new_ranks)
...
def set_directory(directory):
    os.makedirs(directory, exist_ok=True)  # In case other processes already created the folder

    filepath = "%s/%d.log" % (directory, MyLogFormatter.RANK)
    fhandler = logging.FileHandler(filepath)
    fhandler.setFormatter(MyLogFormatter())
...
class InfoFilter(logging.Filter):
    def filter(self, rec):
        return MyLogFormatter.RANK in MyLogFormatter.ALLOWED_RANKS
